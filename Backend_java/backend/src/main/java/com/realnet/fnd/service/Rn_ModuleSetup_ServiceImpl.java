...
//	}
	...
	public Rn_Module_Setup updateById(int id, Rn_Module_Setup moduleRequest) {
		...
		oldModule.setDbName(moduleRequest.getDbName());
		oldModule.setDbPassword(moduleRequest.getDbPassword());
		oldModule.setDbUserName(moduleRequest.getDbUserName());
		oldModule.setMicroservice(moduleRequest.isMicroservice());
		oldModule.setPortaldeployment(moduleRequest.isPortaldeployment());
		oldModule.setReadme(moduleRequest.isReadme());
		oldModule.setParentrepo(moduleRequest.getParentrepo());
		oldModule.setCopy_baseproj(moduleRequest.isCopy_baseproj());
		oldModule.setTesting(moduleRequest.isTesting());
		oldModule.setLoginservice(moduleRequest.isLoginservice());
		oldModule.setType(moduleRequest.getType());
		oldModule.setBackendConfig_ts(moduleRequest.getBackendConfig_ts());
...
//		oldModule.setProject(moduleRequest.getProject()); // no need
	...
	public List<DropDownDTO> getModulesForDropDown() {
		// List<Rn_Module_Setup> moduleList =
		// moduleSetupRepository.findModulesForDropDown();
