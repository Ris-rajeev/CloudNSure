...
import com.realnet.flf.service.Rn_Bcf_TechnologyStack_Service;
...
//	private Rn_Fb_Line_Repository fbLineRepository;
	...
	public Rn_Fb_Header getById(int id) {
		Rn_Fb_Header fb_header = fbHeaderRepository.findById(id).orElse(null);
		return fb_header;
	}
...
//		long accId = loggedInUser.getSys_account().getId();
		...
		oldHeader.setTesting(headerRequest.isTesting());
		oldHeader.setChild_form(headerRequest.isChild_form());
		oldHeader.setBackend_id(headerRequest.getBackend_id());
		boolean add_tomobile = headerRequest.isAdd_tomobile();

		oldHeader.setAdd_tomobile(add_tomobile);
		final Rn_Fb_Header updatedHeader = fbHeaderRepository.save(oldHeader);

		if (add_tomobile) {
			Integer moduleId = headerRequest.getServiceTechid();
			Rn_Module_Setup module = moduleService.getById(moduleId);

			Rn_Fb_Header rn_fb_header = new Rn_Fb_Header();
			rn_fb_header.setUiName(headerRequest.getUiName());
			rn_fb_header.setObjectType("form");
			rn_fb_header.setSubObjectType("only header");

			rn_fb_header.setBuild(false);
			rn_fb_header.setUpdated(false);
			rn_fb_header.setEditable(true);

			rn_fb_header.setModule(module);

			Rn_Fb_Header header = fbHeaderRepository.save(rn_fb_header);

			Rn_wf_lines_3 wf_lines_3 = repo.findheader(updatedHeader.getId()).orElseThrow(null);
			String model = wf_lines_3.getModel();

			Rn_wf_lines_3 lines_3 = new Rn_wf_lines_3();
			lines_3.setHeader_id(header.getId());
			lines_3.setModel(model);
			repo.save(lines_3);
...
//	}
	...
	@Override
	public boolean deleteheaderbyid(int id) {
		if (!fbHeaderRepository.existsById(id)) {
			throw new ResourceNotFoundException(Constant.NOT_EXIST_EXCEPTION);
		}
		Rn_Fb_Header fb_header = fbHeaderRepository.findById(id)
	...
	// default value
	@Override
	public Rn_Fb_Header saveWireframe(Rn_WireFrameDTO wireframeDTO, String formType, int moduleId) {
...
//		String subObjectType = wireframeDTO.getSubObjectType();
		...
		BackendConfig_t backendConfig_t = backendConfig_Repository.findById(wireframeDTO.getBackend_id()).get();

		Rn_Fb_Header rn_fb_header = new Rn_Fb_Header();
		// rn_fb_header.setModule_id(module_id);
		rn_fb_header.setUiName(uiName);
		rn_fb_header.setTechStack(backendConfig_t.getTechstack());
		rn_fb_header.setObjectType("form");
		rn_fb_header.setSubObjectType("only header");
		...
		rn_fb_header.setChild_form(wireframeDTO.isChild_form());
		rn_fb_header.setBackend_id(wireframeDTO.getBackend_id());
...
//        header.setFormCode(headerDetails.getFormCode());
//        header.setJspName(headerDetails.getJspName());
//        header.setControllerName(headerDetails.getControllerName());
//        header.setServiceName(headerDetails.getServiceName());
//        header.setServiceImplName(headerDetails.getServiceImplName());
//        header.setDaoName(headerDetails.getDaoName());
//        header.setDaoImplName(headerDetails.getDaoImplName());
//        header.setBuild(headerDetails.isBuild());
//        header.setUpdated(headerDetails.isUpdated());
...
//        header.setConvertedTableName(headerDetails.getConvertedTableName());
		...
		header.setChild_form(headerDetails.isChild_form());
